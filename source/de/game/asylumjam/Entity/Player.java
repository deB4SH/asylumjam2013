...
public class Player extends Entity {
    ...
    public void render(SpriteBatch spriteBatch) {
        ...
        CharSequence text = "" + (float)Math.toDegrees(Math.atan2(Gdx.input.getY() - (getPosition().y - Game1.camera.position.y) , Gdx.input.getX() - (getPosition().x - Game1.camera.position.x)));
        ...
        double v1 = Gdx.input.getY() - (getPosition().y - Game1.camera.position.y);
        double v2 = Gdx.input.getX() - (getPosition().x - Game1.camera.position.x);
        float rotation = (float)Math.toDegrees(Math.atan2( v1 ,v2 ));

        spriteBatch.draw(super.getTexture(), getPosition().x - Game1.camera.position.x, getPosition().y - Game1.camera.position.y,16,16,32,32,1,1,rotation,0,0,32,32,false,false);
